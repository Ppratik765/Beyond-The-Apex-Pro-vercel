...
const TYRE_COLORS = { 'SOFT': '#ff3b30', 'MEDIUM': '#ffcc00', 'HARD': '#ffffff', 'INTERMEDIATE': '#43a047', 'WET': '#0057e7', 'UNKNOWN': '#888' };
...
function Dashboard({ session, handleLogout }) {
  ...
  const [inputs, setInputs] = useState({ year: new Date().getFullYear(), race: '', session: '', drivers: 'VER, LEC' });
  ...
  const [animOrigin, setAnimOrigin] = useState("top right");
  ...
  useEffect(() => {
      ...
      setShowMobileCharts(false); // Reset on new search
  ...
  const getSimulatedStandings = () => {
      ...
      Object.keys(predictions).forEach(round => {
          ...
          if(roundPred.race) {
              Object.entries(roundPred.race).forEach(([pos, driverCode]) => {
                  ...
                  if(driver) { driver.points += points; const team = simWcc.find(t => t.team === driver.team); if(team) team.points += points; }
              });
          }
          if(roundPred.sprint) {
      ...
      simWdc.sort((a,b) => b.points - a.points); simWcc.sort((a,b) => b.points - a.points);
      simWdc.forEach((d,i) => d.position = i+1); simWcc.forEach((t,i) => t.position = i+1);
  ...
  const updatePrediction = (type, pos, driverCode) => {
      ...
      setPredictions(prev => ({
          ...prev, [roundData.round]: { ...prev[roundData.round], [type]: { ...(prev[roundData.round]?.[type] || {}), [pos]: driverCode } }
      }));
  };

  const getAvailableDrivers = (round, type, currentPos) => {
      const picked = predictions[round]?.[type] || {};
      const pickedCodes = Object.entries(picked).filter(([pos, code]) => parseInt(pos) !== currentPos && code).map(([_, code]) => code);
  ...
  const fetchRaceOverview = async () => {
      ...
      try {
            ...
            setRaceLapData(res.data.data.laps); setStintData(res.data.data.stints);
            setRaceWinner({ name: res.data.data.race_winner, label: res.data.data.winner_label });
            setRaceWeather(res.data.data.weather); setRaceInsights(res.data.data.ai_insights || []);
  ...
  const handleMainAction = () => { if(isRaceOrPractice) fetchRaceOverview(); else fetchDetailedTelemetry(); };
  const resetAllCharts = () => { [deltaChartRef, speedChartRef, throttleChartRef, brakeChartRef, rpmChartRef, longGChartRef].forEach(ref => { if (ref.current) ref.current.resetZoom(); }); };

  const getDatasets = (metric) => {
    if (!telemetryData) return [];
    return Object.keys(telemetryData.drivers).map((key, idx) => ({
        label: key, data: telemetryData.drivers[key]?.telemetry[metric] || [],
        borderColor: DRIVER_COLORS[idx % DRIVER_COLORS.length], borderWidth: 2, pointRadius: 0, tension: 0, hoverBorderWidth: 3
    }));
  };
  ...
  const raceDistributionData = useMemo(() => {
    ...
    activeDrivers.forEach((driver, dIdx) => {
        ...
        driverLaps.forEach(lap => {
            ...
            borderCmds.push(isSelected ? 2 : 1); radiuses.push(isSelected ? 6 : 3.5);
        });
    });
  ...
  const handleDistributionClick = (event, elements) => {
    ...
    if (rawPointData && rawPointData.rawLapData) {
        ...
        setSelectedLaps(prevSelected => {
            ...
            if (newSelection.length > 0) fetchDetailedTelemetry(newSelection); else setTelemetryData(null);
            return newSelection;
        });
    }
  };
  ...
  const telemetryOptions = useMemo(() => ({
    animation: false, maintainAspectRatio: false, responsive: true, interaction: { mode: 'index', intersect: false },
    onHover: (e, elements) => { if (elements && elements.length > 0) setHoverIndex(elements[0].index); else setHoverIndex(null); },
  ...
  const pctTelemetryOptions = useMemo(() => ({ ...telemetryOptions, scales: { ...telemetryOptions.scales, y: { min: 0, max: 105, ticks: { color: '#888' }, grid: { color: COLORS.grid } } } }), [telemetryOptions]);

  const distributionOptions = useMemo(() => ({
      animation: false, maintainAspectRatio: false, responsive: true, onClick: handleDistributionClick,
      plugins: { legend: { display: false }, zoom: false, tooltip: { backgroundColor: 'rgba(20, 20, 30, 0.9)', titleColor: COLORS.neon, bodyFont: { family: '"Titillium Web"' }, callbacks: { label: (ctx) => `${ctx.raw.rawLapData.driver} L${ctx.raw.rawLapData.lap_number}: ${formatTime(ctx.raw.rawLapData.lap_time_seconds)} (${ctx.raw.rawLapData.compound})` } } },
      scales: { x: { type: 'linear', offset: false, title: { display: true, text: 'DRIVERS', color: '#666', font:{weight:'bold'} }, ticks: { color: 'white', font: { size: 14, weight: 'bold', family: '"Titillium Web"' }, stepSize: 1, callback: (val) => activeDrivers[Math.round(val)] || '', autoSkip: false }, grid: { display: false }, min: -0.5, max: activeDrivers.length - 0.5 }, y: { ticks: { color: '#888', callback: (val) => formatTime(val), stepSize: 0.1 }, grid: { color: COLORS.grid }, title: { display: true, text: 'LAP TIME (m:ss.ms)', color: '#666', font:{weight:'bold'} } } }
  }), [activeDrivers]);
  ...
  const getTrackMapData = () => {
      ...
      const s1 = [], s2 = [], s3 = [];
      t.distance.forEach((d, i) => { const pt = { x: t.x[i], y: t.y[i] }; if(d <= s1End) s1.push(pt); if(d >= s1End && d <= s2End) s2.push(pt); else if(i > 0 && t.distance[i-1] < s1End && d > s1End) s2.push(pt); if(d >= s2End) s3.push(pt); else if(i > 0 && t.distance[i-1] < s2End && d > s2End) s3.push(pt); });
      const datasets = [ { label: 'S1', data: s1, borderColor: COLORS.s1, borderWidth: isMapExpanded?5:4, pointRadius: 0, showLine: true }, { label: 'S2', data: s2, borderColor: COLORS.s2, borderWidth: isMapExpanded?5:4, pointRadius: 0, showLine: true }, { label: 'S3', data: s3, borderColor: COLORS.s3, borderWidth: isMapExpanded?5:4, pointRadius: 0, showLine: true } ];
      if(hoverIndex !== null && t.x[hoverIndex]) { datasets.push({ label: 'Car', data: [{ x: t.x[hoverIndex], y: t.y[hoverIndex] }], backgroundColor: 'white', borderColor: 'black', borderWidth: 2, pointRadius: isMapExpanded ? 10 : 6 }); }
      return { datasets };
  };
  ...
  const toggleMapExpand = () => {
      if (isMapExpanded) {
          ...
          setAnimOrigin(`${originX}px ${originY}px`); setIsClosing(true); setTimeout(() => { setIsMapExpanded(false); setIsClosing(false); }, 300);
      } else {
          if (mapBtnRef.current) {
              const rect = mapBtnRef.current.getBoundingClientRect(); setButtonCoords(rect);
              const defaultX = window.innerWidth - 550; const defaultY = 120;
              const originX = rect.left - defaultX + (rect.width / 2); const originY = rect.top - defaultY + (rect.height / 2);
              setAnimOrigin(`${originX}px ${originY}px`); setMapPosition({ x: defaultX, y: defaultY });
          }
          setIsMapExpanded(true);
      }
  };

  const handleMouseDown = (e) => { isDragging.current = true; dragOffset.current = { x: e.clientX - mapPosition.x, y: e.clientY - mapPosition.y }; document.addEventListener('mousemove', handleMouseMove); document.addEventListener('mouseup', handleMouseUp); };
  const handleMouseMove = (e) => { if (!isDragging.current) return; setMapPosition({ x: e.clientX - dragOffset.current.x, y: e.clientY - dragOffset.current.y }); };
  const handleMouseUp = () => { isDragging.current = false; if (widgetRef.current) { setMapSize({ width: widgetRef.current.offsetWidth, height: widgetRef.current.offsetHeight }); } document.removeEventListener('mousemove', handleMouseMove); document.removeEventListener('mouseup', handleMouseUp); };
  ...
  // We use a small inline style trick to hide charts based on state if on mobile
  const isMobile = window.innerWidth <= 768; 
  const chartsVisible = !isMobile || showMobileCharts;
  ...
  return (
    <div className="dashboard-container" style={{paddingBottom: '50px'}}>
                  ...
                  {predictionMode && currentRace ? (
                      <div style={styles.predictorContainer}>
                          <div style={{...styles.card, gridColumn: 'span 2', display: 'flex', flexDirection: 'column', maxHeight: '600px'}}>
                              ...
                              <div style={{display:'flex', gap:'20px', overflowY: 'auto', paddingRight: '5px', flex: 1, paddingBottom:'20px', flexDirection: 'column'}}>
                                  ...
                                  {currentRace.is_sprint && ( <div style={{flex:1}}> <h5 style={styles.colHeader}>SPRINT PREDICTION (TOP 8)</h5> <div style={styles.predGrid}> {[...Array(8)].map((_, i) => { const pos = i + 1; const currentVal = predictions[currentRace.round]?.sprint?.[pos] || ''; const available = getAvailableDrivers(currentRace.round, 'sprint', pos); return ( <div key={i} style={styles.predRow}> <span style={{color:'#666', width:'25px'}}>{pos}</span> <select style={styles.predSelect} value={currentVal} onChange={(e) => updatePrediction('sprint', pos, e.target.value)} > <option value="" style={{backgroundColor: '#2b2b3d', color: 'white'}}>Select Driver</option> {currentVal && <option value={currentVal} style={{backgroundColor: '#2b2b3d', color: 'white'}}>{currentVal}</option>} {available.map(d => <option key={d.code} value={d.code} style={{backgroundColor: '#2b2b3d', color: 'white'}}>{d.code}</option>)} </select> <span style={{color: COLORS.neon, fontSize:'0.8em'}}>+{SPRINT_POINTS[pos]}pts</span> </div> ); })} </div> </div> )}
                              </div>
                          </div>
                          <div style={{...styles.card, maxHeight: '600px', display: 'flex', flexDirection: 'column'}}> <h4 style={styles.cardTitle}>WDC (PREDICTED)</h4> <div style={styles.standingsList}> {displayedStandings.wdc.map((d, i) => ( <div key={d.code} style={styles.standingRow}> <span style={{color: COLORS.neon, width:'25px'}}>{i+1}</span> <span style={{flex:1, textAlign:'left'}}>{d.code}</span> <b>{d.points}</b> </div> ))} </div> </div>
      ...
      {(isMapExpanded || isClosing) && telemetryData && ( <div ref={widgetRef} style={{ ...styles.floatingWidget, left: mapPosition.x, top: mapPosition.y, width: `${mapSize.width}px`, height: `${mapSize.height}px`, resize: 'both', overflow: 'hidden', transformOrigin: animOrigin, animation: isClosing ? 'morphOut 0.3s cubic-bezier(0.16, 1, 0.3, 1) forwards' : 'morphIn 0.3s cubic-bezier(0.16, 1, 0.3, 1) forwards' }}> <div style={styles.floatingHeader} onMouseDown={handleMouseDown} title="Drag Header to Move | Drag Bottom-Right to Resize"> <span>📍 TRACK MAP</span> <button onClick={toggleMapExpand} style={styles.closeBtn}>✕</button> </div> <div style={{height: 'calc(100% - 50px)', padding:'10px'}}> <Scatter data={getTrackMapData()} options={trackMapOptions} /> </div> </div> )}

      <div className="dashboard-header" style={styles.topBar}>
        <span style={{ color: COLORS.textDim, fontSize: '0.9em' }}> Logged in as: <b style={{ color: COLORS.neon }}>{session?.user?.user_metadata?.full_name || session?.user?.email}</b> </span>
        <div style={{display: 'flex', alignItems: 'center', gap: '20px'}}>
             <div className="map-trigger" style={styles.mapButton} onClick={fetchChampionshipData} title="Championship Standings"> 🏆 </div>
             {telemetryData && !isMapExpanded && ( <div ref={mapBtnRef} style={{position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', cursor:'pointer'}} onClick={toggleMapExpand} className="map-trigger"> <div style={styles.tooltipAbove}>CLICK TO EXPAND</div> <div style={styles.mapButton}> <Scatter data={getTrackMapData()} options={{...trackMapOptions, animation: false}} /> </div> </div> )}
             <button onClick={handleLogout} style={styles.logoutBtn}>Log Out</button>
        </div>
      </div>

      <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'25px'}}> <h1 style={{margin:0, fontSize: '2em', fontStyle:'italic', fontWeight:'800', letterSpacing:'-1px'}}> 🏎️ Beyond The Apex <span style={{color: COLORS.primary, fontSize:'0.5em', verticalAlign:'text-top'}}>PRO</span> </h1> <div style={styles.controlsBar}> {telemetryData && !isRaceOrPractice && <span>🖱️ <b>Drag</b> to Zoom | ⇧ <b>Shift+Drag</b> Pan | <span style={{cursor:'pointer', color: COLORS.primary, fontWeight:'bold'}} onClick={resetAllCharts}>⟲ Reset All</span></span>} {isRaceOrPractice && !telemetryData && <span>🖱️ <b>Click dots</b> to select specific laps.</span>} {isRaceOrPractice && telemetryData && <span>Analyzing Selection. <span style={{cursor:'pointer', color: COLORS.primary, fontWeight:'bold'}} onClick={() => {setTelemetryData(null); setSelectedLaps([]);}}>✕ Clear</span></span>} </div> </div>
      ...
      {isRaceOrPractice && raceLapData && !loading && (
          <div className="dashboard-grid-race">
               <div style={{...styles.card, display: 'flex', flexDirection: 'column', height: '600px'}}> <h4 style={styles.cardTitle}>LAP TIME DISTRIBUTION</h4> <div style={{ flex: 1, minHeight: 0, position: 'relative', overflow: 'hidden' }}> <Scatter ref={distributionChartRef} options={distributionOptions} data={raceDistributionData} /> </div> <div style={{marginTop:'15px', display:'flex', gap:'15px', justifyContent:'center', fontSize:'0.8em', color: COLORS.textDim}}> {Object.entries(TYRE_COLORS).map(([compound, color]) => (compound !== 'UNKNOWN' && <div key={compound} style={{display:'flex', alignItems:'center'}}><div style={{width:'8px', height:'8px', borderRadius:'50%', backgroundColor:color, marginRight:'6px', boxShadow:`0 0 5px ${color}`}}></div>{compound}</div>))} </div> </div>
               <div style={{display:'flex', flexDirection:'column', gap:'20px'}}> <div style={{...styles.card, border: `1px solid ${COLORS.neon}`, textAlign:'center', background: 'linear-gradient(180deg, rgba(0, 243, 255, 0.1), transparent)'}}> <h4 style={{margin:'0 0 5px 0', color: COLORS.neon, fontSize:'0.7em', letterSpacing:'2px'}}>{raceWinner?.label || "WINNER"}</h4> <div style={{fontSize:'1.5em', fontWeight:'800', color:'white', textShadow: `0 0 10px ${COLORS.neon}`}}>{raceWinner?.name || "N/A"}</div> </div> <div style={styles.card}> <h4 style={styles.cardTitle}>TYRE STRATEGY</h4> {stintData && activeDrivers.map(d => ( <div key={d} style={{marginBottom:'25px'}}> <div style={{fontWeight:'bold', marginBottom:'8px', color:'white', fontSize:'1.1em', display:'flex', alignItems:'center'}}><span style={{width:'3px', height:'15px', background: COLORS.primary, marginRight:'8px'}}></span>{d}</div> <div style={{display:'flex', width:'100%', height:'20px', background:'#111', borderRadius:'4px', overflow:'hidden', marginBottom:'8px'}}> {stintData[d] && stintData[d].map((stint, i) => ( <div key={i} style={{flex: stint.end - stint.start + 1, backgroundColor: getTyreColor(stint.compound), borderRight: '1px solid #1a1a1a'}} title={`${stint.compound} (${stint.end - stint.start + 1} laps)`}></div> ))} </div> </div> ))} </div> {raceInsights.length > 0 && ( <div style={{...styles.card, borderTop: `3px solid ${COLORS.primary}`}}> <h4 style={styles.cardTitle}>🤖 Tyre Deg Insights</h4> <div style={{fontSize:'0.85em', color: COLORS.textDim, lineHeight:'1.5'}}> {raceInsights.map((insight, i) => (<div key={i} style={{marginBottom:'8px', paddingBottom:'8px', borderBottom:`1px solid ${COLORS.grid}`}}>{insight}</div>))} </div> </div> )} </div>
          </div>
      )}
          ...
          <div className="dashboard-grid-telemetry">
              ...
              <div style={{display:'flex', flexDirection:'column', gap:'20px'}}>
                ...
                <div className="mobile-chart-toggle" onClick={() => setShowMobileCharts(!showMobileCharts)}>
                    {showMobileCharts ? 'Hide Telemetry Charts 🔼' : 'Show Telemetry Charts 📊'}
                </div>
              </div>
          </div>
      )}
...
const styles = {
    // ... [Keep existing non-modal styles] ...
